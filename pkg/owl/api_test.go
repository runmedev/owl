...
func TestPublicAPIWithOwlCueMapsRedisRequirement(t *testing.T) {
	t.Parallel()
...
`)),
	...
	assert.Equal(t, "localhost", byName["REDIS_HOST"].Value)
	assert.Equal(t, owl.TypeUniverseRedis, byName["REDIS_HOST"].Type)
	assert.Equal(t, `universe/redis("queues").host`, byName["REDIS_HOST"].Field.String())
	assert.Equal(t, "Redis server hostname", byName["REDIS_HOST"].Description)
	assert.Equal(t, owl.VisibilityLiteral, byName["REDIS_HOST"].Visibility)

	assert.Equal(t, "6379", byName["REDIS_PORT"].Value)
	assert.Equal(t, `universe/redis("queues").port`, byName["REDIS_PORT"].Field.String())
	assert.Equal(t, "Redis server port", byName["REDIS_PORT"].Description)

	assert.Equal(t, "[unset]", byName["REDIS_PASSWORD"].Value)
	assert.Equal(t, `universe/redis("queues").password`, byName["REDIS_PASSWORD"].Field.String())
	assert.Equal(t, "Redis password", byName["REDIS_PASSWORD"].Description)
	assert.Equal(t, owl.VisibilityUnresolved, byName["REDIS_PASSWORD"].Visibility)
	assert.Equal(t, "owl.cue", byName["REDIS_PASSWORD"].Origin.Name)
...
func TestV2PublicAPICompileSurface(t *testing.T) {
	...
	var _ owl.StoreOption = owl.WithOwlCue("owl.cue", strings.NewReader(""))
